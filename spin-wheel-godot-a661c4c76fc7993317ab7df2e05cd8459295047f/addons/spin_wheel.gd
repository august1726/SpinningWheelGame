	...
	if is_spin == false:
		...
		tween.connect("finished", func():
			...
			if old_rotation_degrees > 360:
				...
				$background/front.rotation_degrees = rad_
			)
		...
		tween.tween_property($background/front, "rotation_degrees", reward_position +  360 * speed * power , 3).set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_CIRC)
		tween.finished
